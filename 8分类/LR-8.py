...
from sklearn.linear_model import LogisticRegression  # 使用GLM模型
...
# 使用GLM模型（LogisticRegression）
glm_model = LogisticRegression(max_iter=500, random_state=42, multi_class='multinomial', solver='lbfgs')

# 训练模型
glm_model.fit(xtrain_s, ytrain)

# 预测测试集
y_pred = glm_model.predict(xtest_s)
y_pred_proba = glm_model.predict_proba(xtest_s)
...
for i in range(ytest_one_rf.shape[1]):
    glm_FPR[i], glm_TPR[i], _ = roc_curve(ytest_one_rf[:, i], y_pred_proba[:, i])
    glm_AUC[i] = auc(glm_FPR[i], glm_TPR[i])
print("ROC AUC for each class:", glm_AUC)

# 计算宏平均ROC曲线和AUC
glm_FPR_final = np.unique(np.concatenate([glm_FPR[i] for i in range(ytest_one_rf.shape[1])]))
glm_TPR_all = np.zeros_like(glm_FPR_final)
for i in range(ytest_one_rf.shape[1]):
    glm_TPR_all += np.interp(glm_FPR_final, glm_FPR[i], glm_TPR[i])
glm_TPR_final = glm_TPR_all / ytest_one_rf.shape[1]
glm_AUC_final = auc(glm_FPR_final, glm_TPR_final)
print(f"Macro Average AUC with LR: {glm_AUC_final}")

# 绘制ROC曲线
plt.figure(figsize=(10, 5), dpi=300)
# 使用不同的颜色和线型
plt.plot(glm_FPR[0], glm_TPR[0], color='b', linestyle='-', label='Class 1 ROC  AUC={:.4f}'.format(glm_AUC[0]), lw=0.8)
plt.plot(glm_FPR[1], glm_TPR[1], color='g', linestyle='-', label='Class 2 ROC  AUC={:.4f}'.format(glm_AUC[1]), lw=0.8)
plt.plot(glm_FPR[2], glm_TPR[2], color='r', linestyle='-', label='Class 3 ROC  AUC={:.4f}'.format(glm_AUC[2]), lw=0.8)
plt.plot(glm_FPR[3], glm_TPR[3], color='c', linestyle='-', label='Class 4 ROC  AUC={:.4f}'.format(glm_AUC[3]), lw=0.8)
plt.plot(glm_FPR[4], glm_TPR[4], color='m', linestyle='-', label='Class 5 ROC  AUC={:.4f}'.format(glm_AUC[4]), lw=0.8)
plt.plot(glm_FPR[5], glm_TPR[5], color='y', linestyle='-', label='Class 6 ROC  AUC={:.4f}'.format(glm_AUC[5]), lw=0.8)
plt.plot(glm_FPR[6], glm_TPR[6], color='#FFA500', linestyle='-', label='Class 7 ROC  AUC={:.4f}'.format(glm_AUC[6]), lw=0.8)
plt.plot(glm_FPR[7], glm_TPR[7], color='#9467bd', linestyle='-', label='Class 8 ROC  AUC={:.4f}'.format(glm_AUC[7]), lw=0.8)
# 宏平均ROC曲线
plt.plot(glm_FPR_final, glm_TPR_final, color='#000000', linestyle='-', label='Macro Average ROC  AUC={:.4f}'.format(glm_AUC_final), lw=1)
# 45度参考线
