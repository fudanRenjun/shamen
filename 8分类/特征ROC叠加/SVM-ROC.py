...
# 导入CSV文件
data = pd.read_csv('E:/RS/sm/ALL-Feature/SVM/SVM-20.csv')
...
for i in range(1, 21):
    ...
    for label in range(8):  # 假设标签范围为0-7
        ...
        # 训练XGBoost模型
        model = SVC(C=100, gamma='scale', kernel='rbf', probability=True)
...
print(auc_df)
auc_df.to_csv('E:/RS/sm/ALL-Feature/SVM/SVM-20-ROC.csv', index=False, encoding='utf-8-sig')